...
def list_expenses(include_deleted: bool = False, db: Session = Depends(get_db)) -> list[Expense]:
...
def delete_expense(expense_id: str, db: Session = Depends(get_db)) -> Expense:
    expense = db.get(Expense, expense_id)
    if expense is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found")
